...
const CommandTable = ({
  ...
  const applyCmd = (apply, cmd) => dispatch({
  ...
  const columns = [{
    ...
    render: (apply, record) => {
      return (
        <Switch size="default" checked={apply} size="small"
    ...
    render: (update, record) => {
      return (
        ...
        ><Button size="small" type="danger" ghost className="commands-del">{i18n('table.action.del')}</Button>
        </Popconfirm>
      );
    }
  }];
