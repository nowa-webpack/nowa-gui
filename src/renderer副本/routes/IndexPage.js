...
class IndexPage extends Component {
  ...
  render() {
    const { showPage, dispatch, version } = this.props;
    ...
    const mainbody = <WelcomePage version={version} dispatch={dispatch} />;

    return (
      <Dropzone className="drop-ctn" 
        onDrop={this.onDrop} 
        onDragOver={this.onDragOver}
        onDragLeave={this.onDragLeave}>
        <Layout className="container" id="main-ctn">
          <Header className="top-bar">
            { showPage > 0 && <div className="bar-bd" /> }
            <div className="logo" onClick={() => shell.openExternal('https://nowa-webpack.github.io/')} />
            { showPage > 0 && <div className="proj-path">
              uxcore-demo
              <span>(/User/layne/NowaProjects/uxcore-demo)</span>
            </div>}
